...
namespace YmtSystem.CrossCutting.ExpressionTreeSerialization
{
    public class ExpressionSerializer
    {
        ...
        public XElement Serialize(Expression expresion)
        {
            return GenerateXmlFromExpressionCore(expresion);
        }
        ...
        private XElement GenerateXmlFromExpressionCore(Expression e)
        {
            ...
            foreach (var prop in properties)
            {
                try
                {
                    ...
                    else if (prop.Name == "NodeType")
                    {
                        val = e.NodeType;
                        isConstant = e.NodeType == ExpressionType.Constant;
                    }
                    else
                    {
                        val = prop.GetValue(e, BindingFlags.Default, null, null, CultureInfo.CurrentCulture);
                    }

                    //object val = prop.GetValue(e, null); // original;
        ...
        {
            return Converters.Select(converter => converter.Serialize(e)).FirstOrDefault(result => result != null);
        }

        private static string GetNameOfExpression(Expression e)
        {
        ...
        private XElement GenerateXmlFromTypeCore(Type type)
        {
            ...
            else
            {
                ...
                {
                    if (type.FullName != null) return new XElement("Type", new XAttribute("Name", type.FullName));
                }
            }
            return null;
        }
        ...
        private Type ParseAnonymousTypeFromXmlCore(XElement xElement)
        {
            ...
            return _resolver.GetOrCreateAnonymousTypeFor(name, properties.ToArray(), ctr_params.ToArray());
        }
