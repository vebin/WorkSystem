namespace YmtSystem.Domain.Specifications
{
    ...
    public class AndNotSpecification<T> : CompositeSpecification<T>
    {
        ...
        {
            var bodyNot = Expression.Not(Right.SatisfiedBy().Body);
            var bodyNotExpression = Expression.Lambda<Func<T, bool>>(bodyNot, Right.SatisfiedBy().Parameters);

            return Left.SatisfiedBy().And(bodyNotExpression);
        }
        #endregion
    }
}
